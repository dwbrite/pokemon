...
class ControlMap {
    ...
    init {
        for (key in Controls.InputDir.values()) {
            this[key] = false
        }
        for (key in Controls.InputKey.values()) {
            this[key] = false
        }
    }
    ...
    operator fun set(key: Controls.InputDir, value: Boolean) {
        inputDir[key] = value
    }

    operator fun set(key: Controls.InputKey, value: Boolean) {
        inputKey[key] = value
    }
}